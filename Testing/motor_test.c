...
// Updated drv8434s_read_reg with two-transaction read
                      ...
                      uint8_t *value)
{
    ...
    // The DRV8434S prepares the data but doesn't return it yet
    int flag = spi_transceive(spi_dev, spi_cfg, &tx_spi_buf_set, &rx_spi_buf_set);
    if (flag < 0) {
        printk("Read transaction 1 failed: %d\n", flag);
        return flag;
    }
    
    // First transaction returns garbage/zeros
    printk("[DEBUG] Trans 1: TX=[0x%02X 0x%02X] RX=[0x%02X 0x%02X]\n", 
           tx_buf[0], tx_buf[1], rx_buf[0], rx_buf[1]);
    
    k_usleep(10);  // Small delay between transactions
    
    // ==========================================
    // TRANSACTION 2: Send dummy to get data
    // ==========================================
    
    // Send NOP (read from address 0x00) to clock out the real data
    tx_buf[0] = 0x40;  // W=1 (read), Address=0x00
    tx_buf[1] = 0x00;
    
    flag = spi_transceive(spi_dev, spi_cfg, &tx_spi_buf_set, &rx_spi_buf_set);
    if (flag < 0) {
        printk("Read transaction 2 failed: %d\n", flag);
        return flag;
    }
    
    // Second transaction returns the actual register data
    printk("[DEBUG] Trans 2: TX=[0x%02X 0x%02X] RX=[0x%02X 0x%02X]\n", 
           tx_buf[0], tx_buf[1], rx_buf[0], rx_buf[1]);
    
    // The actual register value is in the lower byte
    *value = rx_buf[1];
